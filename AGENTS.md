...
- [ ] **Plan R3: Remediation 3** *(in progress — plan written, 4 GitHub issues triaged + labeled + milestoned to `v1.0.0 closed-test gate`. R3-01 (#2 backgrounding state loss, blocker) merged to `main` via PR #5 (615 → 619 tests). R3-02 (#4 THORN_DAMAGE wiring + LIFESTEAL visibility, major) merged to `main` via PR #6 (619 → 622 tests). R3-03 (#1 COMPLETE_RESEARCH mission false trigger, major) merged to `main` via PR #7 (622 → 627 tests). **R3-04 (#3 battle bottom control-bar overflow, minor) fix landed on branch `fix/3-bottom-bar-overflow` 2026-05-19 evening (627 tests still green, pure Compose UI change verified on-device only) — awaiting PR review + merge.**)*
