...
- [ ] **Plan R3: Remediation 3** *(in progress — plan written, 4 GitHub issues triaged + labeled + milestoned to `v1.0.0 closed-test gate`. R3-01 (#2 backgrounding state loss, blocker) merged to `main` via PR #5 (615 → 619 tests). **R3-02 (#4 THORN_DAMAGE wiring + LIFESTEAL visibility, major) fix landed on branch `fix/4-thorn-damage-lifesteal` 2026-05-19 (619 → 622 tests, both `testDebugUnitTest` and `bundleRelease` green) — awaiting PR review + merge.** R3-03 (#1 COMPLETE_RESEARCH false trigger, major) still pending. R3-04 (#3 bottom-bar scroll) needs reporter clarification.)*
