...
class ResolveStats {
    ...
    operator fun invoke(
        ...
        fun total(type: UpgradeType) = (workshopLevels[type] ?: 0) + (inRoundLevels[type] ?: 0)
        fun ws(type: UpgradeType) = workshopLevels[type] ?: 0

        return ResolvedStats(
            damage = ZigguratBaseStats.BASE_DAMAGE *
                (1 + total(UpgradeType.DAMAGE) * 0.02),
            attackSpeed = ZigguratBaseStats.BASE_ATTACK_SPEED *
                (1 + total(UpgradeType.ATTACK_SPEED) * 0.015),
            ...
            maxHealth = ZigguratBaseStats.BASE_HEALTH *
                (1 + total(UpgradeType.HEALTH) * 0.03),
