 ...
 */
@Singleton
internal class RealBillingClientAdapter @Inject constructor(
    ...
    private val purchasesUpdatedListener = PurchasesUpdatedListener { result, purchases ->
        val deferred = pendingPurchase ?: run {
    ...
    override suspend fun launchPurchase(
    ...
    ): StartPurchaseResult = purchaseMutex.withLock {
        val rawProductDetails = productDetails.rawRef as? ProductDetails
        ...
        val offerToken: String? = rawProductDetails.subscriptionOfferDetails
            ?.firstOrNull()
            ?.offerToken
    ...
    private fun SdkProductType.toPlay(): String = when (this) {
        SdkProductType.INAPP -> ProductType.INAPP
        SdkProductType.SUBS -> ProductType.SUBS
    }

    private fun BillingResult.toSdk(): SdkBillingResult = when (responseCode) {
        BillingClient.BillingResponseCode.OK -> SdkBillingResult.Ok
        BillingClient.BillingResponseCode.USER_CANCELED -> SdkBillingResult.UserCanceled(debugMessage)
        BillingClient.BillingResponseCode.SERVICE_DISCONNECTED -> SdkBillingResult.ServiceDisconnected(debugMessage)
        BillingClient.BillingResponseCode.SERVICE_UNAVAILABLE -> SdkBillingResult.ServiceUnavailable(debugMessage)
        BillingClient.BillingResponseCode.BILLING_UNAVAILABLE -> SdkBillingResult.BillingUnavailable(debugMessage)
        BillingClient.BillingResponseCode.ITEM_UNAVAILABLE -> SdkBillingResult.ItemUnavailable(debugMessage)
        BillingClient.BillingResponseCode.ITEM_ALREADY_OWNED -> SdkBillingResult.ItemAlreadyOwned(debugMessage)
        BillingClient.BillingResponseCode.DEVELOPER_ERROR -> SdkBillingResult.DeveloperError(debugMessage)
        BillingClient.BillingResponseCode.NETWORK_ERROR -> SdkBillingResult.NetworkError(debugMessage)
        else -> SdkBillingResult.Other(responseCode, debugMessage)
    }

    private fun ProductDetails.toSdk(): SdkProductDetails {
        val price: String = oneTimePurchaseOfferDetails?.formattedPrice
