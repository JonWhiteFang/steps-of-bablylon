...
internal class RewardAdManagerImpl @Inject constructor(
    ...
    override suspend fun showRewardAd(placement: AdPlacement): AdResult = sessionMutex.withLock {
        ...
        runCatching { consentManager.ensureInitialized(activity) }
            .onFailure { Log.w(TAG, "Consent init failed; proceeding with canRequestAds()=${consentManager.canRequestAds()}", it) }
        ...
        val loaded = when (val load = adapter.loadAd(adUnitId)) {
            is SdkAdLoadResult.Success -> load.ad
        ...
        return@withLock when (val show = adapter.showAd(activity, loaded)) {
            is SdkAdShowResult.Rewarded -> AdResult.Rewarded
            is SdkAdShowResult.Dismissed -> AdResult.Cancelled
    ...
    private fun adUnitIdForPlacement(placement: AdPlacement): String = when (placement) {
     ...
     */
    private fun SdkAdLoadResult.Error.toUserMessage(): String = when (code) {
     ...
     */
    private fun SdkAdShowResult.Error.toUserMessage(): String = when (code) {
