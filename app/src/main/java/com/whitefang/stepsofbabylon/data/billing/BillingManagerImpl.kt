...
internal class BillingManagerImpl @Inject constructor(
    ...
    override suspend fun purchase(product: BillingProduct): PurchaseResult = sessionMutex.withLock {
        ...
        val productDetails = when (detailsResult) {
            is QueryProductDetailsResult.Success -> detailsResult.products.firstOrNull()
        ...
        val launch = adapter.launchPurchase(
            activity = activity,
        ...
        return@withLock when (launch) {
            is StartPurchaseResult.Completed -> handleCompletedPurchase(product, launch.purchase)
     ...
     */
    private suspend fun creditWallet(product: BillingProduct, purchase: SdkPurchase) {
        when (product) {
            ...
            BillingProduct.AD_REMOVAL -> playerProfileDao.updateAdRemoved(true)
     ...
     */
    private suspend fun finalizePurchase(product: BillingProduct, purchase: SdkPurchase) {
        when (product) {
            BillingProduct.GEM_PACK_SMALL,
            BillingProduct.GEM_PACK_MEDIUM,
            BillingProduct.GEM_PACK_LARGE -> {
    ...
    private fun obfuscatedAccountIdHex(): String {
        ...
        val uuid = prefs.getString(KEY_ANTI_FRAUD_UUID, null)
    ...
    private fun BillingProduct.sdkProductType(): SdkProductType = when (this) {
