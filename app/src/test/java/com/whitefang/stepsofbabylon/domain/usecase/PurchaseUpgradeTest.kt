...
class PurchaseUpgradeTest {
    ...
    @Test
    fun `insufficient steps returns false without mutating either repo`() = runTest {
    ...
    @Test
    fun `successful purchase uses atomic repo method and does not call spendSteps directly`() = runTest {
        val wallet = playerRepo.profile.value.toWallet()
        assertTrue(sut(UpgradeType.DAMAGE, 0, wallet))
    ...
    @Test
    fun `purchase skips atomic call when wallet fast-fail trips`() = runTest {
    ...
    @Test
    fun `two concurrent purchases on exactly sufficient balance - only one succeeds`() = runTest {
