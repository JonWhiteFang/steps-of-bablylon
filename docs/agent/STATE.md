...
## Current objective
- **Smoke-test v5 (versionCode 5) on internal track + promote to closed.** RO-11 implementation complete on `main` across 3 commits + plan write-up + this doc-sync (commits `d3dc4d6`, `a4eca72`, `14b0665`, `28337e5`, `6b754c9`, `93f6ae8`). Test count 572 → 609 (+37, vs ~36 plan target). Phase A wired the 7 simple multipliers (DAMAGE / HEALTH / CASH / CRITICAL / REGEN / STEP_EFFICIENCY / UW_COOLDOWN); Phase B wired WAVE_SKIP and gated AUTO_UPGRADE_AI + ENEMY_INTEL with a "Coming Soon" UI badge + VM-level defensive guard; Phase C shipped `DescribeUpgradeEffect` use case + the in-round upgrade-menu "Now → Next" readout the user originally asked for. `./run-gradle.sh test` + `./run-gradle.sh bundleRelease` both BUILD SUCCESSFUL. Next: bump `versionCode 4 → 5`, upload to internal track, run the 8 acceptance smoke checks per `docs/plans/plan-RO-11-labs-wiring.md` § 8, then promote internal v5 → closed.
...
1. **Build + upload v5 — immediate.** Bump `versionCode 4 → 5` in `app/build.gradle.kts`, run `./run-gradle.sh bundleRelease`, upload to Play Console internal track.
2. **On-device smoke test v5.** 8 acceptance checks per `docs/plans/plan-RO-11-labs-wiring.md` § 8: DAMAGE_RESEARCH L5 visibly hits harder; HEALTH_RESEARCH L5 ≥+25 % HP; CASH_RESEARCH L5 ≥+25 % per-kill cash; CRITICAL_RESEARCH × CRITICAL_CHANCE crit damage ≥+30 %; STEP_EFFICIENCY L5 → 100 sensor steps credited as ≥110; UW_COOLDOWN L10 → ~70 % baseline cooldown; WAVE_SKIP L5 → HUD opens on Wave 6; in-round upgrade menu shows non-empty "Now → Next" line per row; AUTO_UPGRADE_AI + ENEMY_INTEL rows render "COMING SOON" badge with no Start button. Plus RO-08 + RO-09 regression checks.
3. **Plan 31 Phase G2 — closed testing track.** If smoke green, promote internal v5 → closed. Recruit ≥12 testers (Gmail addresses), distribute opt-in URL, wait ≥14 calendar days.
4. **Plan 31 Phase H — Pre-launch report review.** Auto-runs Firebase Test Lab on every internal-track AAB upload. Review + fix anything critical (bump versionCode → bundleRelease → re-upload to closed track).
5. **Plan 31 Phase I — production access application + rollout.** After ≥14 days closed testing with ≥12 testers, apply for production access. Google review 1–3 days. Then promote closed → production with staged rollout (5–10 % → 100 %). Tag v1.0.0 in git post-rollout.

## Next actions (explicit order)
1. **(Build + upload, immediate)** Bump `versionCode 4 → 5`, run `./run-gradle.sh bundleRelease`, upload to internal track.
2. **(Smoke test)** Install v5 from Play Console internal track on a physical device. Run the 8 acceptance checks per plan-RO-11 § 8. If anything fails, file follow-up before promoting to closed.
3. **(External)** Promote internal v5 → closed testing in Play Console. Recruit ≥12 testers (Gmail addresses), distribute opt-in URL.
...
- Critical path: 01→…30→R→R2→ Battle Step Rewards → Phase A done → B.1 done → B.2 done (RO-02 complete) → B.3 done (RO-03 complete) → B.4–B.5 (deferred post-launch) → C.2 PRs done → C.4 done → C.5 PRs 1+2+3 done → C.6 PRs 1+2+3 done → battle-step-credit hotfix done → RO-08 done (4-fix upgrade-wiring bundle) → RO-09 done (chrono fix + fortune stacking + drive-by) → **RO-11 done** (Phase A 7 simple Labs multipliers + Phase B WAVE_SKIP + Coming Soon gate + Phase C in-round readout, 572 → 609 tests) → Plan 31 (Phases A–G done; smoke test PASSED 2026-05-18; v4 superseded by v5) → **v5 upload + smoke-test + Phase G2 closed track** (≥14 days, ≥12 testers) → Phases H+I production access → tag v1.0.0
- Last run: 2026-05-19 early hours — RO-11 implementation landed across 6 commits on `main` (`d3dc4d6` Phase A.1 stats research, `a4eca72` Phase A.2 cash + UW cooldown research, `14b0665` Phase A.3 STEP_EFFICIENCY, `28337e5` Phase B.1 WAVE_SKIP, `6b754c9` Phase B.2 Coming Soon gate, `93f6ae8` Phase C in-round readout). Test count 572 → 609. `./run-gradle.sh test --rerun-tasks` BUILD SUCCESSFUL with zero failures; `./run-gradle.sh bundleRelease` BUILD SUCCESSFUL. Pending: doc-sync commit then v5 build + upload.
