...
## Current objective
- Phase B.2 PR 2 (RO-02 site #1) is complete — `AwardBattleSteps` now credits battle-earned Steps through an atomic `DailyStepDao.creditBattleStepsAtomic` (cap check + counter increment + wallet credit in one SQLite transaction). Two RO-02 atomic sites done; 3 remain (StepCrossValidator, ClaimMilestone, endRound wrapper).
...
- **463 JVM tests** green (+51 vs pre-Phase-A 412 baseline; +8 vs pre-B.2 455 baseline).
...
- Phase B core refactors (@Transaction for 5 multi-write sites, resilient endRound, FollowOnPipeline extraction, UpdateMissionProgress use case) are debt, not blockers. B.1 TimeProvider landed. B.2 PRs 1–2 (`PurchaseUpgrade`, `AwardBattleSteps`) landed; 3 RO-02 sites remain (StepCrossValidator, ClaimMilestone, endRound wrapper). B.3 PR 1 landed; B.3 PR 2 (`onCleared` guard) remains.

## Top priorities (next 5)
1. Phase B.2 PR 3 — `StepCrossValidator` repo-level `AppDatabase.withTransaction { }` around each of the 3 parallel graduated-response branches. Different idiom from PRs 1–2 but licensed by the RO-02 non-goal list.
2. Phase B.2 PR 4 — `ClaimMilestone` atomic `@Transaction` (composite method on `MilestoneDao` taking `PlayerProfileDao`). Same pattern as B.2 PRs 1–2.
3. Phase B.2 PR 5 — wrap `runEndRoundPersistence` in a Room `@Transaction` (single-call-site change thanks to B.3 PR 1).
4. Phase B.3 PR 2 — `onCleared` guard using `ProcessLifecycleOwner.lifecycleScope` so mid-battle deep-link navigation no longer silently discards round progress.
5. Phase C.2 — Cosmetic rendering pipeline PRs 1–2 (ship one cosmetic end-to-end). Release critical path.

## Next actions (explicit order)
1. B.2 PR 3 — `StepCrossValidator` repo-level `AppDatabase.withTransaction { }` around each graduated-response branch.
2. B.2 PR 4 — atomic `@Transaction` for `ClaimMilestone` (composite method on `MilestoneDao` taking `PlayerProfileDao`). Same pattern as B.2 PRs 1–2.
3. B.2 PR 5 — wrap `runEndRoundPersistence` body in a Room `@Transaction`. Single-call-site change.
4. B.3 PR 2 — `onCleared` guard so mid-nav deep-links no longer silently lose round progress.
5. Open ADR-0005 (Billing SDK) and ADR-0006 (Ad SDK) stubs, then land C.5 + C.6.
6. C.2 cosmetic pipeline can land anywhere after B.1 — pick first cosmetic (gap_analysis §5.2 proposes jade-ziggurat recolour).
...
- Critical path: 01→…→30→R→R2→ Battle Step Rewards → **Phase A done** → B.1 done → B.2 PRs 1–2 done → B.3 PR 1 done → B.2 PRs 3–5 + B.3 PR 2 + B.4–B.5 → C → D → 31
- Last run: 2026-05-07 (Phase B.2 PR 2 — atomic @Transaction for AwardBattleSteps; 461 → 463 tests, all green; lintDebug green; two RO-02 atomic sites now landed, three remain)
