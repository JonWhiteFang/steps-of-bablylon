...
- **App launcher icon landed (2026-05-12)** — vector adaptive icon: solid deep-lapis `#0E2247` background + 5-tier stepped-ziggurat foreground with vertical Gold → SandStone → lightened-DeepBronze gradient (all brand colors from `Color.kt`). Resources in `res/drawable/ic_launcher_{background,foreground}.xml` + `res/mipmap-anydpi-v26/ic_launcher{,_round}.xml`. `AndroidManifest.xml` wired with `android:icon` + `android:roundIcon` on `<application>`. minSdk=34 — no raster fallbacks needed; adaptive XML is the single source of truth. Closes the "No app icon resources" debt item.
- **Play Store 512×512 hi-res icon rendered (2026-05-13)** — `docs/release/store-assets/play-store-icon-512.png` (3.8 KB). Generated by `tools/render_play_store_icon.py` from the same 108-viewport coordinates as the adaptive icon XML, supersampled 4× then LANCZOS-downsampled. One fewer Plan 31 raster blocker.
- **Play Store 1024×500 feature graphic landed (2026-05-13)** — `docs/release/store-assets/play-store-feature-graphic-1024x500.png` (621.5 KB). User supplied a 1376×768 pixel-art Tower of Babel scene (`StepsOfBabylonArt.png`); center-vertical-cropped (y=48..720) to match Play Store's 2.048 aspect, then LANCZOS-downscaled. Tower, character, swirl-sky, path, and framing ruins all preserved. Source PNG kept in-repo for future re-crops. Plan 31 raster blocker count: 3 → 1 (only screenshots remain).
...
- Last run: 2026-05-13 (Play Store 1024×500 feature graphic: user-provided `StepsOfBabylonArt.png` 1376×768 pixel-art Tower of Babel scene center-vertical-cropped + LANCZOS-downscaled to `docs/release/store-assets/play-store-feature-graphic-1024x500.png` 621.5 KB; release-checklist line ticked; Plan 31 raster blocker count 3 → 1).
