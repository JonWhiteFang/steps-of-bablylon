...
- Last run: 2026-05-13 (Play Store 512×512 hi-res icon: rendered via `tools/render_play_store_icon.py` Pillow-only script from the same coords as the adaptive icon XML; output `docs/release/store-assets/play-store-icon-512.png` 3.8 KB; release-checklist.md app-icon line ticked; one fewer Plan 31 raster blocker).
