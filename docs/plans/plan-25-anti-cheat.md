...
**Status:** Not Started
**Dependencies:** Plan 05 (Google Fit Integration)
...
Harden the anti-cheat system beyond the basic rate limiting and daily ceiling from Plan 04. Add accelerometer pattern analysis to detect phone shakers, strengthen Google Fit cross-validation, prevent Activity Minute gaming, and implement overlap deduction.
...
### Task 2: Enhanced Google Fit Cross-Validation
...
- Implement graduated response:
  ...
  - Repeated: reduce credited rate to Google Fit value
  - Persistent: cap at Google Fit reported steps only

---

### Task 3: Activity Minute Gaming Prevention

Create `data/googlefit/ActivityMinuteAntiCheat.kt`:
...
├── ActivityMinuteValidator.kt  (update — overlap deduction)
...
- Google Fit cross-validation uses graduated response for repeated discrepancies
