...
presentation/battle/BattleScreen.kt                # Compose wrapper: AndroidView + overlays (HUD, pause, post-round), auto-pause. R3-04 (#3): bottom control-row modifier chain rewrapped to align → windowInsetsPadding(navigationBars) → padding(bottom=24dp) → horizontalScroll(rememberScrollState()) → background → padding(h=12dp,v=8dp) so the 6 buttons (3× speed + Pause + Upgrade + Overdrive) clear the system gesture handle and gracefully scroll horizontally on phones too narrow to fit them all (e.g. Pixel 6, 411dp).
